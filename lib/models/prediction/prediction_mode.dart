...
///
/// This enum corresponds to the PredictionMode enum in the Combustion Inc iOS/macOS BLE SDK. The mode determines what
/// type of predictions the probe will generate during cooking.
enum PredictionMode {
  /// No prediction mode is active.
  ///
  /// The probe is not generating predictions. This typically occurs when no target temperature has been set.
  none,

  /// Time-to-removal prediction mode.
  ///
  /// The probe predicts when the food will reach the target temperature and should be removed from heat. This is the
  /// standard cooking mode.
  timeToRemoval,

  /// Removal and resting prediction mode.
  ///
  /// The probe predicts both when to remove the food from heat and tracks temperature during the resting period. This
  /// mode accounts for carryover cooking that occurs after removal from heat.
  removalAndResting,

  /// Reserved mode for future use.
  ///
  /// This mode is reserved for future SDK features and should not be used in current implementations.
  reserved;

  /// Converts a raw integer value from the native SDK to [PredictionMode].
  ///
  /// The raw value corresponds to the UInt8 rawValue from the iOS/macOS SDK's PredictionMode enum.
  ///
