...
class _HistoricalTemperatureChartState extends State<HistoricalTemperatureChart> {
  ...
  ///
  /// When zooming in via button (not gesture), maintain focus on recent data by adjusting pan offset to keep the most
  /// recent data visible.
  void _zoomIn() {
    setState(() {
      ...
      // If we're using default timespan and haven't manually panned, adjust pan to keep recent data in view when
      // zooming in
  ...
  ///
  /// For very large datasets, shows a smaller recent window for better readability. For smaller datasets, shows more or
  /// all data.
  ...
  ///
  /// This is a helper method to avoid duplicating the line building logic when we need to access the data boundaries in
  /// zoom calculations.
  ...
  Widget build(BuildContext context) {
    ...
    double centerX;
    if (_panOffset == 0.0) {
      // No user panning - bias towards most recent data For default timespan, center closer to the right (most recent)
