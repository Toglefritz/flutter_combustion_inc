...
///
/// This widget can display both real-time temperature streams and historical temperature logs, with support for virtual
...
class _TemperatureGraphState extends State<TemperatureGraph> {
  ...
  ///
  /// Sets up subscriptions to both virtual temperature updates (core, surface, ambient) and physical temperature
  /// updates (T1-T8 sensors). Data points are only added when not showing historical data to avoid mixing real-time and
  /// historical data.
  ...
  ///
  /// Monitors the probe's session information availability and automatically attempts to load historical data when a
  /// session becomes available. Clears any previous errors when session info becomes available.
  void _startSessionInfoStream() {
    _sessionInfoSubscription = widget.probe.sessionInfoStream.listen((Map<String, dynamic> sessionInfo) async {
      if (mounted) {
        ...
        // If session info just became available and we don't have historical data yet, try to load it But don't show
        // errors immediately - wait for user to explicitly request historical data
  ...
  ///
  /// Used for automatic loading when session info becomes available. Does not display errors to the user - they will
  /// only see errors when explicitly requesting historical data via the UI.
  Future<void> _loadHistoricalDataSilently() async {
    ...
    try {
      ...
      _logSubscription = log.dataStream.listen(
        (ProbeLogDataPoint dataPoint) {
          if (mounted) {
            setState(() {
        ...
        onError: (Object error) {
          // Silently handle stream errors during background loading
          debugPrint('Temperature log stream error (silent load): $error');
        },
      );
      ...
      debugPrint('Failed to load temperature log silently: $e');
    }
  }

  /// Starts a timer to periodically retry loading historical data silently.
  ///
  /// Retries every 5 seconds for up to 12 attempts (1 minute total). Automatically stops when data is successfully
  /// loaded, session info is lost, or the maximum number of attempts is reached.
  ...
  ///
  /// This method is called when the user explicitly requests historical data (e.g., by pressing the retry button). It
  /// shows loading states and error messages to the user. If session info is not available, it attempts to refresh it
  /// before trying to load the temperature log.
  ...
  ///
  /// Only attempts to load data if session information is available. This method is called when the user presses the
  ...
  ///
  /// Converts temperatures to the current unit setting and maintains a rolling window of the most recent
  /// [maxTimeWindowSeconds] seconds of data for performance.
  ...
  ///
  /// Processes all 8 physical temperature sensors (T1-T8) and converts them to the current unit setting. Maintains a
  /// rolling window of the most recent [maxTimeWindowSeconds] seconds of data for each sensor for performance.
  ...
  ///
  /// Takes a temperature value in Celsius and converts it to the user's preferred unit (Celsius or Fahrenheit) based on
  /// the current setting.
  ...
  ///
  /// Maintains a rolling window of the most recent [maxTimeWindowSeconds] seconds of data by removing points that fall
  /// outside the time window. This ensures the live chart always shows the most recent 30 seconds of data.
  ...
  ///
  /// This method is called periodically by the timer to ensure that even when no new data arrives, the chart maintains
  /// the 30-second rolling window.
  ...
  ///
  /// Returns the appropriate symbol ('°C' or '°F') based on the current temperature unit setting for display in the UI.
